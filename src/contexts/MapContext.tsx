...
interface MapContextType {
  ...
  userRegion: string; // 사용자 시도별 위치를 저장할 상태
...
export const MapProvider = ({ children }: MapProviderProps) => {
  ...
  const [userRegion, setUserRegion] = useState(''); // 사용자 시도별 위치
  ...
  useEffect(() => {
    if (!navigator.geolocation) {
      setUserRegion('Geolocation을 지원하지 않습니다.');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        setUserRegion('위치 정보를 가져올 수 없습니다.');
      },
    );
  }, []);
  ...
  return (
    <MapContext.Provider
      value={{ map, userRegion, userCoordinates, setMapInstance }}
    >
      {children}
    </MapContext.Provider>
  );
